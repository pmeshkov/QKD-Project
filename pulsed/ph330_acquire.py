...
def validate(config):
    ...
    if not isinstance(channels, list) or len(channels) != 2:
        raise ValueError("Provide exactly two detector configurations.")
    ids = [c.get("channel") for c in channels]
    if any(type(i) is not int or not 0 <= i < 4 for i in ids) or len(set(ids)) != 2:
...
def configure(api, cfg):
    ...
    if not math.isfinite(resolution) or resolution <= 0 or resolution * 32768 < 1e12 / cfg["laser_hz"]:
        raise ValueError("T3 microtime range does not cover the full 500 ns period; increase binning.")
...
def run(api, cfg, seconds, destination, acquire):
    ...
    try:
        ...
        if abs(before["sync_hz"] / cfg["laser_hz"] - 1) > 0.05:
            raise RuntimeError("SYNC rate is not within 5% of 2 MHz; check laser and trigger settings.")
        if any(before["input_hz"][d["channel"]] <= 0 for d in cfg["detectors"]):
            raise RuntimeError("Both detector rates must be nonzero before this g2 acquisition.")
            ...
            with (folder / "events.t3raw").open("xb") as stream:
                ...
                try:
                    ...
                    if not math.isfinite(period) or abs(period*cfg["laser_hz"]-1) > 0.05:
                        raise RuntimeError("Measured SYNC period is invalid or differs from 500 ns.")
