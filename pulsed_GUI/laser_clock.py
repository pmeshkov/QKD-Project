...
def clock_plan(frequency_hz, high_ns):
    ...
    if not math.isfinite(frequency_hz) or not 1_000 <= frequency_hz <= 10_000_000:
        raise ValueError("Characterization frequency must be 1 kHz through 10 MHz.")
...
def run_clock(args, plan, stop_event=None):
...
def main(argv=None, stop_event=None):
    ...
    try:
        return run_clock(args, plan, stop_event=stop_event)
    except Exception as exc:
