...
# Ask user whether to save before interacting with hardware
user_input = input("Do you want to save the sweep data to a CSV file? (y/n): ").strip().lower()
save_data = user_input in ['y', 'yes']

# Generate timestamped filename
timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
csv_filename = f"Vpi_Sweep_{timestamp}.csv"

# Lists to hold data in memory for plotting
target_voltages = []
pfi1_rates = []
pfi2_rates = []
...
with nidaqmx.Task() as ao_task, nidaqmx.Task() as ci_task0, nidaqmx.Task() as ci_task1:
    ...
    if save_data:
        ...
        print("Data will NOT be saved to a file (memory only).\n")
    ...
    file = None
    try:
        # Conditionally open CSV in write mode
        if save_data:
        ...
        while current_target_v <= STOP_V:
            ...
            if firstrun:
                # 5 seconds of sleep time before the first voltage 
            ...
            # Save data to memory arrays for plotting
            target_voltages.append(current_target_v)
            pfi1_rates.append(hz0)
            pfi2_rates.append(hz1)
            
            # 7. Log to CSV and flush to disk immediately to prevent data loss on crash (if saving)
            if save_data:
...
# Create the plot using the lists held in memory
