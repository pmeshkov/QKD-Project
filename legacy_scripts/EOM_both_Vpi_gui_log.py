...
# --- Define GUI Widgets ---
# Text box for ao0 voltage input
ax_volt_0 = fig.add_axes([0.10, 0.1, 0.25, 0.05])
volt_box_0 = TextBox(ax_volt_0, 'EOM0 (ao0) V: ', initial=str(init_voltage_0))

# Text box for ao1 voltage input
ax_volt_1 = fig.add_axes([0.45, 0.1, 0.25, 0.05])
volt_box_1 = TextBox(ax_volt_1, 'EOM1 (ao1) V: ', initial=str(init_voltage_1))

# Button for toggling the scale
ax_btn = fig.add_axes([0.75, 0.1, 0.15, 0.05])
...
with nidaqmx.Task() as ao_task, nidaqmx.Task() as ci_task0, nidaqmx.Task() as ci_task1:
    ...
    def submit_volt_0(text):
        try:
            ...
            # Update state array and write both channels to hardware
